...
export default function CalendarPage() {
  ...
  // Fetch ALL restock items (keep your existing restock fetch logic)
  useEffect(() => {
    const fetchRestockItems = async () => {
      try {
        setLoadingRestock(true);
  ...
  function refreshCalendar() {
    const api = calendarRef.current?.getApi?.();
    if (api) api.refetchEvents();
    loadUpcomingEvents(); // Refresh upcoming events when calendar refreshes
    setRefreshUpcomingEvents(prev => prev + 1); // Trigger upcoming events refresh
  }
  ...
  return (
    <div className="min-h-screen bg-[#EFEFEF] p-4 lg:p-6">
      ...
      {/* Calendar Controls Row - UPDATED STYLES */}

      {/* Calendar Controls Row - FULL WIDTH LAYOUT */}
      {/* Calendar Controls Row - RESPONSIVE LAYOUT */}
      <div className="mb-6">
        {/* Desktop Layout - Buttons in single row (hidden on mobile) */}
        <div className="hidden xl:block">
          <div className="w-full flex justify-between items-center gap-4 mb-4">
            {/* View Buttons Container - LEFT SIDE */}
            <div className="w-2/3 relative bg-white overflow-hidden rounded-lg flex justify-start items-stretch border border-gray-200">
              {/* Month Button */}
              <div className={`flex-1 ${currentView === "dayGridMonth" ? "bg-[#238D88]" : ""}`}>
                <button
                  onClick={() => handleViewChange("dayGridMonth")}
                  className={`w-full h-12 py-2 justify-center items-center gap-2.5 inline-flex transition-colors ${currentView === "dayGridMonth"
              ...
              <div className={`flex-1 ${currentView === "timeGridWeek" ? "bg-[#238D88]" : ""}`}>
                <button
                  onClick={() => handleViewChange("timeGridWeek")}
                  className={`w-full h-12 py-2 justify-center items-center gap-2.5 inline-flex transition-colors ${currentView === "timeGridWeek"
                    ? "bg-[#238D88] text-white font-semibold"
                    : "bg-white text-black hover:bg-gray-50 font-medium"
              ...
              <div className={`flex-1 ${currentView === "timeGridDay" ? "bg-[#238D88]" : ""}`}>
                <button
                  onClick={() => handleViewChange("timeGridDay")}
                  className={`w-full h-12 py-2 justify-center items-center gap-2.5 inline-flex transition-colors ${currentView === "timeGridDay"
                    ? "bg-[#238D88] text-white"
                    : "bg-white text-black hover:bg-gray-50"
                    } text-sm font-dm-sans font-medium`}
                >
            ...
            <div className="w-1/3">
              <button
                onClick={() => setModalOpen(true)}
                className="w-full h-12 px-6 bg-[#F3BE08] overflow-hidden rounded-lg justify-center items-center gap-2.5 inline-flex hover:bg-amber-500 transition-colors"
              >
                <div className="justify-center flex flex-col text-[#232527] text-base font-dm-sans font-semibold">
                  Add Event +
                </div>
              </button>
            </div>
        ...
        {/* Mobile Layout - Buttons in grid sections (shown on mobile) */}
        <div className="xl:hidden">
          <div className="grid grid-cols-1 gap-6 mb-6">
            {/* Calendar Section */}
            <div>
              {/* View Buttons Container - SPANS FULL CALENDAR WIDTH */}
              <div className="bg-white rounded-lg border border-gray-200 overflow-hidden flex justify-start items-stretch mb-4">
                {/* Month Button */}
                <div className={`flex-1 ${currentView === "dayGridMonth" ? "bg-[#238D88]" : ""}`}>
                  <button
                    onClick={() => handleViewChange("dayGridMonth")}
            ...
            <div>
              {/* Add Event Button - SPANS FULL UPCOMING EVENTS WIDTH */}
              <button
                onClick={() => setModalOpen(true)}
                className="w-full h-12 px-6 bg-[#F3BE08] overflow-hidden rounded-lg justify-center items-center gap-2.5 inline-flex hover:bg-amber-500 transition-colors mb-4"
              >
                <div className="justify-center flex flex-col text-[#232527] text-base font-dm-sans font-semibold">
                  Add Event +
                </div>
        ...
        {/* Month/Year Navigation - FULL WIDTH (both desktop and mobile) */}
        <div className="flex items-center justify-between bg-white rounded-lg shadow-sm border p-4">
          <div className="flex items-center gap-4">
            <button
              onClick={handlePrevMonth}
      ...
      {/* Restock Items Section (keep your existing restock section) */}
    ...
    >
      {refreshingRestock ? (
...

        {
          loadingRestock ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 border-2 border-dashed border-[#F3BE08] rounded-lg">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="60"
                height="80"
                    ...
                    <div className="flex items-start justify-between gap-3 mb-3">
                      ...
                      <label className="relative inline-flex items-center cursor-pointer flex-shrink-0">
                        <input
                          type="checkbox"
                          checked={item.reminderEnabled}
                          onChange={() => {/* keep your existing toggle handler */ }}
                          className="sr-only peer"
                        />
                        <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-amber-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-amber-400"></div>
                      </label>
      ...
      < ReminderModal
        ...
        onSelectAlert={() => {/* keep your existing handler */ }}
        event={
          selectedRestockItem
            ? {
              title: `Restock: ${selectedRestockItem.productName}`,
